...
class BizLedgerSummary(models.Model):
    _name = 'biz.ledger.summary'
...
class BizLedger(models.Model, LedgerStatementMixin):
    _name = 'biz.ledger'
    ...
    def unlink(self):
        # Allow deleting directly from the ledger view: securely cancel and
        # delete the underlying Journal Entry.
        ...
        # biz.ledger is a SQL view; returning True bypasses generic SQL deletion.
        return True
...
class BizManualEntryWizard(models.TransientModel):
    _name = 'biz.manual.entry.wizard'
    _description = 'Create Manual Ledger Entry'

    partner_id = fields.Many2one('res.partner', string='Partner', required=True)
    date = fields.Date(string='Date', required=True, default=fields.Date.context_today)
    narration = fields.Char(string='Narration / Label', default='Manual Settlement')
    ...
    def action_post_entry(self):
        ...
        # Reconcile selected items with the new lines.
        if self.line_ids:
