...
class BizBillPaymentVendor(models.Model):
    _name = 'biz.bill.payment.vendor'
    ...
    transaction_ids = fields.One2many('biz.bill.payment.transaction', 'vendor_id', string='Transactions')
    ledger_ids = fields.One2many('biz.bill.vendor.ledger', 'vendor_id', string='Ledger')
...
class BizMoneyAssociate(models.Model):
    _name = 'biz.money.associate'
    ...
    phone = fields.Char(string='Phone')
    notes = fields.Text(string='Notes')
    transaction_ids = fields.One2many('biz.bill.payment.transaction', 'received_by_id', string='Handled Transactions')
    ledger_ids = fields.One2many('biz.associate.ledger', 'associate_id', string='Ledger')
...
class BizBillPaymentTransaction(models.Model, LedgerStatementMixin):
    _name = 'biz.bill.payment.transaction'
    ...
    vendor_id = fields.Many2one('biz.bill.payment.vendor', string='Bill/Payment Vendor', required=True)
    ...
    associate_ledger_ids = fields.One2many('biz.associate.ledger', 'transaction_id', string='Associate Ledger')
    ...
    def _compute_ledger_count(self):
        for rec in self:
            rec.ledger_count = len(rec.vendor_ledger_ids) + len(rec.associate_ledger_ids)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('biz.bill.payment.transaction') or 'New'
            if vals.get('vendor_id') and not vals.get('deduction_percent'):
                vendor = self.env['biz.bill.payment.vendor'].browse(vals['vendor_id'])
...
class BizBillVendorLedger(models.Model):
    _name = 'biz.bill.vendor.ledger'
    _description = 'Bill/Payment Vendor Ledger'
    _order = 'date desc, id desc'

    transaction_id = fields.Many2one('biz.bill.payment.transaction', string='Transaction', ondelete='cascade')
    vendor_id = fields.Many2one('biz.bill.payment.vendor', string='Vendor', required=True, ondelete='cascade')
    ...
    def _compute_debit_credit(self):
        for line in self:
            if line.entry_type in ('deduction', 'cash_received'):
                line.debit = 0.0
...
class BizAssociateLedger(models.Model, LedgerStatementMixin):
    _name = 'biz.associate.ledger'
    _description = 'Money Associate Ledger'
    _order = 'date desc, id desc'

    transaction_id = fields.Many2one('biz.bill.payment.transaction', string='Transaction', ondelete='cascade')
    associate_id = fields.Many2one('biz.money.associate', string='Associate', required=True, ondelete='cascade')
    date = fields.Date(string='Date', required=True)
    entry_type = fields.Selection([
        ('cash_received', 'Cash Received'),
...
class BizBillVendorLedgerSummary(models.Model):
    _name = 'biz.bill.vendor.ledger.summary'
    _description = 'Bill/Payment Vendor Ledger Summary'
    _auto = False
    _order = 'vendor_id'

    vendor_id = fields.Many2one('biz.bill.payment.vendor', string='Vendor', readonly=True)
...
class BizAssociateLedgerSummary(models.Model):
    _name = 'biz.associate.ledger.summary'
    _description = 'Money Associate Ledger Summary'
    _auto = False
    _order = 'associate_id'

    associate_id = fields.Many2one('biz.money.associate', string='Associate', readonly=True)
    debit = fields.Float(string='Total Debit', readonly=True)
    credit = fields.Float(string='Total Credit', readonly=True)
    balance = fields.Float(string='Net Balance', readonly=True)
...
class BizBillPaymentEntryWizard(models.TransientModel):
    _name = 'biz.bill.payment.entry.wizard'
    ...
    received_by_id = fields.Many2one('biz.money.associate', string='Received By')
    to_associate_id = fields.Many2one('biz.money.associate', string='Transfer To')
    actual_cash_received = fields.Float(string='Cash Received')
    # Carrying agent payment
    carrying_agent_id = fields.Many2one('biz.carrying.agent', string='Carrying Agent')
    agent_payment_source = fields.Selection([
        ('vendor', 'Paid from business funds'),
        ('associate', 'Paid by an associate'),
    ...
    def _onchange_transaction_id(self):
        ...
        if self.entry_type == 'receive_payment':
            remaining = tx.expected_cash_amount - (tx.actual_cash_received or 0.0)
            self.actual_cash_received = max(0.0, remaining)
    ...
    def _create_expense(self):
        ...
        expense_line = self.env['biz.associate.ledger'].create({
    ...
    def _create_associate_transfer(self):
        ...
        in_line = self.env['biz.associate.ledger'].create({
