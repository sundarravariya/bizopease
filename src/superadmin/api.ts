...
export const sa = {
  ...
  resetPassword: (id: string, newPassword: string) => call(`/workspaces/${id}/reset-password`, 'POST', { newPassword }),
  backupWorkspace: (id: string) => call(`/workspaces/${id}/backup`, 'POST', {}),
  ...
  deleteBackup: (date: string) => call('/backups/delete', 'POST', { date }),
  health: () => call<any>('/health', 'GET'),
  audit: (limit = 200) => call<any[]>(`/audit?limit=${limit}`, 'GET'),
  testSmtp: (to?: string) => call<{ success: boolean; message: string }>('/test-smtp', 'POST', { to }),
  testQueen: () => call<{ success: boolean; db: string; partnerCount: number }>('/test-queen', 'GET'),
