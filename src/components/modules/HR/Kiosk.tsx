...
import { Nfc, LogIn, LogOut, CheckCircle2, AlertCircle, Play, Square, RefreshCw, Smartphone } from 'lucide-react';
...
export default function Kiosk() {
  ...
  useEffect(() => { nfcStatus().then(setNfc); }, []);
  ...
  const loop = useCallback(async () => {
    ...
    while (runRef.current) {
      ...
      await sleep(2600);   // hold the result on screen
    }
  ...
  if (!isNative()) {
    return (
      <div className="max-w-md mx-auto pt-16 text-center">
        ...
        <p className={`text-sm mt-2 ${sub}`}>Open this screen in the Robifel Admin app on an NFC-capable phone to scan employee badges.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-xl mx-auto pb-24 animate-fade-in">
        ...
        {phase === 'ok' && last ? (
          ...
          <>
            <div className="relative mb-4">
              <div className="w-24 h-24 rounded-full bg-[#7367f0]/15 flex items-center justify-center animate-pulse"><Nfc size={48} className="text-[#7367f0]" /></div>
            </div>
      ...
      {!nfc.available && <p className="text-center text-xs text-amber-500 mt-3 flex items-center justify-center gap-1.5"><AlertCircle size={13} /> This phone has no NFC. Use an NFC-capable phone for the kiosk.</p>}
