    ...
    <div className='space-y-6 animate-fade-in max-w-5xl mx-auto'>
      ...
      <div className={`card p-6 ${gc} flex flex-col sm:flex-row items-center gap-5`}>
        ...
        <div>
          ...
          <div className={`text-xs ${ts}`}>{user?.is_admin ? 'Administrator' : 'User'}{user?.company_name ? ` · ${user.company_name}` : ''}</div>
        ...
        <div className={`card p-6 space-y-5 ${gc}`}>
          ...
          <form onSubmit={handleSaveProfile} className='space-y-4'>
            ...
            <div>
              ...
              <input readOnly value={user?.company_name || ''} className={`${inp} w-full opacity-60 cursor-not-allowed`} />
            </div>
            <div className='pt-2'>
