  ...
  const openLedger = async (type: PartyType, party: Party) => {
    ...
    try {
      ...
      if (type === 'vendor') {
        ...
        // Expand each transaction into: 1 outflow row + optional inflow rows
        for (const r of rows || []) {
          ...
          onDeleteEntry={async item => {
            ...
            try {
              if (item.virtualType === 'cash_received') {
                await writeRecord('flipkart.bill.payment.transaction', [item.parentId!], { actual_cash_received: 0, payment_received: false });
              } else if (item.virtualType === 'agent_payment') {
                await writeRecord('flipkart.bill.payment.transaction', [item.parentId!], { agent_payment_amount: 0, carrying_agent_id: false });
              } else {
                const model = activeParty.type === 'vendor' ? 'flipkart.bill.payment.transaction'
                  : activeParty.type === 'associate' ? 'flipkart.associate.ledger' : 'flipkart.agent.ledger';
                if (activeParty.type === 'vendor') {
                  // Cascade: also remove any associate/agent ledger entries referencing this transaction
                  try {
                    const rel = await searchRead<any>('flipkart.associate.ledger', { domain: [['reference', '=', item.title]], fields: ['id'], limit: 0 });
...
const LEDGER_MODELS: Record<PartyType, string> = {
  vendor: 'flipkart.bill.payment.transaction',
  associate: 'flipkart.associate.ledger',
  agent: 'flipkart.agent.ledger',
};
