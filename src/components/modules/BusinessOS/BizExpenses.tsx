...
// Native (non-Flipkart) Expense Manager — pixel-for-pixel the same UI as the
// Flipkart Settlement Expenses screen, repointed to biz.expense / biz.owner /
// biz.money.associate. No Flipkart coupling.
...
export default function BizExpenses() {
  ...
  const loadRefs = useCallback(async () => {
    const [as_, ow, em] = await Promise.allSettled([
      searchRead<Ref>('biz.money.associate', { fields: ['id', 'name'], limit: 0 }),
      searchRead<Ref>('biz.owner',           { fields: ['id', 'name'], limit: 0 }),
  ...
  const loadExpenses = useCallback(async () => {
    ...
    try {
      ...
      const r = await searchRead<any>('biz.expense', {
  ...
  const deleteExpense = async (id: number) => {
    ...
    try {
      await unlinkRecord('biz.expense', [id]);
...
function ManageOwnersSheet({ isDark, owners, onClose, onChanged }: {
  ...
  const addOwner = async () => {
    ...
    setSaving(true);
    try {
      const id = await createRecord('biz.owner', { name: name.trim() });
  ...
  const removeOwner = async (id: number) => {
    ...
    try {
      await unlinkRecord('biz.owner', [id]);
  ...
  const submit = async () => {
    ...
    try {
      const vals: Record<string, any> = {
        ...
        // biz.expense.description is required — default to the category label.
        description: form.description?.trim() || catMeta(form.category).label,
      ...
      await createRecord('biz.expense', vals);
      onSaved();
