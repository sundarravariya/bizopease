...
import { RefreshCw, AlertCircle, BarChart3, ShieldAlert, CheckCircle2 } from 'lucide-react';
...
export default function StockValuation() {
  ...
  const fetchValuations = async () => {
    ...
    setMessage(null);
    try {
      const res = await odooCall<ValuationItem[]>('flipkart.stock.valuation', 'search_read', [[]], {
        fields: [
          'id', 'product_id', 'category_id', 'cost_price',
          'main_qty', 'main_valuation',
          'fbf_qty', 'fbf_valuation',
  ...
  const handleRecalculate = async () => {
    ...
    setMessage(null);
    try {
      await odooCall('flipkart.stock.valuation', 'action_recalculate', [[]], {});
  ...
  const mainWhValue = filtered.reduce((sum, item) => sum + (item.main_valuation || 0), 0);
  const fbfValue = filtered.reduce((sum, item) => sum + (item.fbf_valuation || 0), 0);

  const chartData = [
    { name: 'Main Warehouse', value: mainWhValue },
    { name: 'FBF Locations', value: fbfValue },
  ];
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={`text-xs mt-1 ${isDark ? 'text-[#5a6a8a]' : 'text-gray-500'}`}>
            BOM-exploded inventory valuation showing total capital-at-risk split by storage terminals.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className={`card p-5 space-y-1 ${glassClass}`}>
          <span className={`text-[10px] uppercase tracking-wider font-semibold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Main Warehouse Capital</span>
          <p className={`text-2xl font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>Rs. {mainWhValue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className={`card p-5 space-y-1 ${glassClass}`}>
          <span className={`text-[10px] uppercase tracking-wider font-semibold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>FBF Allocated Capital</span>
          <p className={`text-2xl font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>Rs. {fbfValue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className={`border-b text-xs font-semibold uppercase tracking-wider ${isDark ? 'border-white/5 bg-[#111827]/40 text-gray-400' : 'border-gray-200 bg-gray-50 text-gray-500'}`}>
                    ...
                    <th className="py-4 px-4 text-right">Main Value</th>
                    <th className="py-4 px-4 text-right">FBF Qty</th>
                    <th className="py-4 px-4 text-right">FBF Value</th>
                ...
                <tbody className={`divide-y text-sm ${isDark ? 'divide-white/5' : 'divide-gray-100'}`}>
                  {filtered.map((item) => (
                    <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className={`py-3 px-4 text-right ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{formatRs(item.main_valuation)}</td>
                      <td className={`py-3 px-4 text-right ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{formatQty(item.fbf_qty)}</td>
                      <td className={`py-3 px-4 text-right ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{formatRs(item.fbf_valuation)}</td>
                  ...
                  {filtered.length === 0 && !loading && (
                    <tr>
                      <td colSpan={10} className={`py-10 text-center text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
        ...
        {/* Capital allocation summary chart */}
        <div className={`card p-5 flex flex-col justify-between ${glassClass}`}>
          <div className="space-y-1 mb-4">
            <h3 className={`font-bold text-sm ${isDark ? 'text-white' : 'text-gray-800'}`}>Capital Distribution</h3>
            <p className={`text-[11px] ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Capital split between primary warehouse and marketplace stock.</p>
          </div>
          ...
          <div className={`mt-4 p-3 rounded-xl border flex items-start gap-2.5 bg-amber-500/5 ${isDark ? 'border-amber-500/10' : 'border-amber-500/20'}`}>
            ...
            <span className={`text-[10px] leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              <strong>Note:</strong> Total capital represents Standard Cost price multiplier across leaf component quants, excluding parent assembly margin values.
            </span>
          </div>
        </div>
      </div>
