...
interface VendorLine {
  ...
  received_by_id: [number, string] | false;
...
interface AssociateLine {
  id: number;
  date: string;
  associate_id: [number, string] | false;
  entry_type: string;
  amount: number;
  note: string;
  reference: string;
...
const TXN_STATE: Record<string, { label: string; cls: string }> = {
  ...
  agent_paid: { label: 'Agent Paid', cls: 'bg-blue-500/15 text-blue-400 border border-blue-500/25' },
};
...
export default function UnifiedLedger() {
  ...
  // Create entry form
  const [entryType, setEntryType] = useState<string>('bank_transfer');
  ...
  const loadSummary = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await searchRead<SummaryRow>('flipkart.unified.ledger.summary', {
  ...
  const loadPartnerLines = useCallback(async () => {
    ...
    try {
      const rows = await searchRead<LedgerLine>('business.ledger', {
  ...
  const loadAgentLines = useCallback(async () => {
    ...
    try {
      const rows = await searchRead<AgentLine>('flipkart.agent.ledger', {
  ...
  const loadVendorLines = useCallback(async () => {
    ...
    try {
      const rows = await searchRead<VendorLine>('flipkart.bill.payment.transaction', {
        fields: ['id', 'date', 'vendor_id', 'name', 'transfer_amount', 'deduction_amount', 'expected_cash_amount', 'actual_cash_received', 'received_by_id', 'payment_received', 'state'],
        domain, limit: 0, order: 'date asc',
      });
  ...
  const loadAssociateLines = useCallback(async () => {
    ...
    try {
      const rows = await searchRead<AssociateLine>('flipkart.associate.ledger', {
  ...
  useEffect(() => {
    ...
    else if (tab === 'partner') loadPartnerLines();
    else if (tab === 'agent') loadAgentLines();
    else if (tab === 'vendor') loadVendorLines();
    else if (tab === 'associate') loadAssociateLines();
  }, [tab]);

  const loadVendorTxns = async (vendorId: number) => {
    try {
      const r = await searchRead<{ id: number; name: string }>('flipkart.bill.payment.transaction', {
        fields: ['id', 'name'],
  ...
  // Drill from a summary row into that party's detailed ledger (mirrors action_view_ledger).
  const viewLedger = (r: SummaryRow) => {
    const pid = Array.isArray(r.party_id) ? r.party_id[0] : '';
      ...
      {tab === 'summary' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={loadSummary} disabled={loading} className="btn-secondary text-xs px-3 py-2">
          ...
          <div className={`${cardBg} rounded-2xl p-4`}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={t.key} type="button" onClick={() => { setEntryType(t.key); setEntryForm({ date: new Date().toISOString().slice(0, 10) }); setVendorTxns([]); }}
          ...
          <div className={`${cardBg} rounded-2xl p-5 space-y-4`}>
            ...
            {entryType === 'bank_transfer' && (<>
              <div>
                ...
                <select value={entryForm.vendor_id || ''} onChange={e => { const v = Number(e.target.value); setEntryForm(p => ({ ...p, vendor_id: v || '' })); }} className={`${inp} w-full`}>
            ...
            {entryType === 'agent_payment' && (<>
              ...
              {entryForm.agent_payment_source === 'associate' && (
                <div>
            ...
            <button type="button" onClick={handleCreateEntry} disabled={submitting} className="btn-primary w-full justify-center py-3 text-sm">
      ...
      {tab === 'partner' && (
        <div className="space-y-4">
          <LedgerFilters isDark={isDark} inp={inp}
            ...
            onExport={() => exportCsv('partner-ledger', ['Date','Description','Ref','Debit','Credit','Balance'],
      ...
      {tab === 'agent' && (
        <div className="space-y-4">
          <LedgerFilters isDark={isDark} inp={inp}
            ...
            onExport={() => exportCsv('agent-ledger', ['Date','Agent','Type','Reference','Amount (Rs.)','Notes'],
      ...
      {tab === 'vendor' && (
        <div className="space-y-4">
          <LedgerFilters isDark={isDark} inp={inp}
            ...
            onExport={() => exportCsv('vendor-ledger', ['Date','Reference','Vendor','Bank Transfer','Deducted','Expected Cash','Actual Cash','Received By','Status'],
      ...
      {tab === 'associate' && (
        <div className="space-y-4">
          <LedgerFilters isDark={isDark} inp={inp}
            ...
            onExport={() => exportCsv('associate-ledger', ['Date','Associate','Type','Amount','Reference','Note'],
