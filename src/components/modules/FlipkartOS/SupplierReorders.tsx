...
const FIELDS: string[] = [
  'id', 'sku', 'product_id', 'physical_stock', 'total_fbf_stock', 'incoming_qty',
...
export default function SupplierReorders() {
  ...
  const fetchRecommendations = async () => {
    setLoading(true);
    try {
      const res = await searchRead<ReorderItem>('flipkart.supplier.reorder', {
        fields: FIELDS,
        domain: [],
        limit: 0,
  ...
  const handleGenerate = async () => {
    ...
    setMessage(null);
    try {
      const wizardId = await createRecord('flipkart.supplier.reorder.generate', { warehouse_id: selectedWhId });
      await odooCall('flipkart.supplier.reorder.generate', 'action_generate', [[wizardId]], {});
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr>
                  ...
                  <th className={`${th} text-right`}>FBF Stock</th>
              ...
              <tbody className={`divide-y text-sm ${isDark ? 'divide-[#2a3250]' : 'divide-gray-100'}`}>
                {filtered.map(item => (
                  <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-white/[0.03]' : 'hover:bg-gray-50'} ${item.selected ? 'bg-[#7367f0]/5' : ''}`}>
                    ...
                    <td className={`py-3 px-4 text-right ${st}`}>{item.total_fbf_stock}</td>
