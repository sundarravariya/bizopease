...
export default function DeadStockView() {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const res = await odooCall<DeadStockItem[]>('flipkart.dead.stock', 'search_read', [[]], {
        fields: ['id', 'product_id', 'sku', 'main_stock', 'fbf_stock', 'total_stock', 'stock_value', 'sales_30d', 'sell_through_rate', 'state'],
        limit: 0,
      });
      setItems(res || []);
    } catch (e: any) {
  ...
  const handleGenerate = async () => {
    ...
    setMessage(null);
    try {
      const wizardId = await odooCall<number>('flipkart.dead.stock.generate', 'create', [{ warehouse_id: selectedWhId }]);
      await odooCall('flipkart.dead.stock.generate', 'action_generate', [[wizardId]]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:hidden">
            {filtered.map((item) => (
              <div key={item.id} className={`card p-4 rounded-2xl space-y-3 ${glassClass}`}>
                ...
                <div className={`grid grid-cols-3 gap-2 text-center border-t border-b py-3 my-3 ${isDark ? 'border-white/5' : 'border-gray-100'}`}>
                  <div>
                  ...
                  <div>
                    <span className={`text-[9px] uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>FBF Stock</span>
                    <p className={`font-bold text-sm mt-0.5 ${isDark ? 'text-white' : 'text-gray-800'}`}>{item.fbf_stock}</p>
                  </div>
                  <div>
                    <span className={`text-[9px] uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Total Stock</span>
                    <p className={`font-bold text-sm mt-0.5 ${isDark ? 'text-white' : 'text-gray-800'}`}>{item.total_stock}</p>
                  </div>
                </div>
                ...
                <div className={`grid grid-cols-2 gap-2 text-center pb-2`}>
                  ...
                  <div>
                    <span className={`text-[9px] uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Value</span>
                    <p className="font-bold text-amber-400 text-sm mt-0.5">Rs. {item.stock_value.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
                  </div>
                </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`border-b text-xs font-semibold uppercase tracking-wider ${isDark ? 'border-white/5 bg-[#111827]/40 text-gray-400' : 'border-gray-200 bg-gray-50 text-gray-500'}`}>
                  ...
                  <th className="py-4 px-6 text-right">FBF Stock</th>
              ...
              <tbody className={`divide-y text-sm ${isDark ? 'divide-white/5' : 'divide-gray-100'}`}>
                {filtered.map((item) => (
                  <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className={`py-4 px-6 text-right font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{item.fbf_stock}</td>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={8} className={`py-10 text-center text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
